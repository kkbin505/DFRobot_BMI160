...
DFRobot_BMI160::DFRobot_BMI160(int sda, int scl)
{
  Wire.begin(sda, scl); // 指定引脚
  Obmi160 = (struct bmi160Dev *)malloc(sizeof(struct bmi160Dev));
  // Obmi160->id = BMI160_I2C_ADDR;
  // Obmi160->interface = BMI160_I2C_INTF;
  Oaccel = (struct bmi160SensorData *)malloc(sizeof(struct bmi160SensorData));
...
void DFRobot_BMI160::getMotion6(int16_t *ax, int16_t *ay, int16_t *az,
                                int16_t *gx, int16_t *gy, int16_t *gz)
{
  int16_t accel[6];
  getAccelGyroData(accel);
...
void DFRobot_BMI160::initialize()
{
  this->softReset();
  this->I2cInit();
  uint8_t data;
    ...
    // 设置加速度 ±2g
    data = BMI160_ACCEL_RANGE_2G;
    I2cSetRegs(Obmi160, BMI160_ACCEL_RANGE_ADDR, &data, 1);

    // 设置陀螺 ±250 °/s
    data = BMI160_GYRO_RANGE_250_DPS;
    I2cSetRegs(Obmi160, BMI160_GYRO_RANGE_ADDR, &data, 1);
}
